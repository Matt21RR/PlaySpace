 ...
 */
    ...
    var gmarkers = [];//aqui se almacenan los marcadores para poder ocultarlos
    var map; // almacena el mapa NO QUITAR

    function removerMarcadores(){
        for (let index = 0; index < gmarkers.length; index++) {
            gmarkers[index].setVisible(false);//oculta los marcadores existentes
        }
    }
    function mostrarMarcadores(indiceMarcadores){
        if(indiceMarcadores != -1){
            for (let index = 0; index < indiceMarcadores.length; index++) {
                indice=indiceMarcadores[index];
                gmarkers[indice].setVisible(true);//hace que los marcadores del arreglo se muestren
            }
        }
    }
    function mapa(pos)
    ...
    /************************ Aqui están las variables que te interesan***********************************/
        ...
        console.log("Coordenadas usuario"+centro);
        ...
        if(positionsArray!=-1){//Si existe el array
            ...
            for(i=0;i!=positionsArray.length;i++){
                ...
                if(searchMode==1){
                    var idUbicacion = positionsArray[i][0];
            ...
            if(searchMode==0){//si no se esta realizando una busqueda
                ...
                var latPos = parseFloat(positionsArray[0][0]);//Latitud
                var lonPos = parseFloat(positionsArray[0][1]);//Longitud
                coord = { lat: latPos, lng: lonPos };
                ...
                for(i=0;i!=positionsArray.length;i++){
                    ...
                    google.maps.event.addListener(marker, 'click', (function(marker, i) {
                        return function() {
                            //ventanas(i);
                            pedirInfoUbicacion(marker.typeLocation, marker.id);
