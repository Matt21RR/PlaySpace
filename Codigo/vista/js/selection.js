function selectRadio(primerId,id,cantOp){
  ...
  function selectRadioHint(primerId,id,cantOp){
    i = primerId;
    if (document.querySelector("#_"+id).checked == 1){ //si el cuadro de opcion sobre el que se clica esa ya marcado, desmarcarlo
      document.getElementById("Op"+id).style.border = "unset";
      document.querySelector("#_"+id).checked = 0;
    ...
    //--PARA MOSTRAR O OCULTAR EL TEXTO DE AYUDA
    i = primerId;
    if(document.getElementById("textoAyuda"+id).style.display == "block"){
      
      document.getElementById("textoAyuda"+id).style.display = "none";
    }
    else{
      while (i !=(cantOp+1)){
        document.getElementById("textoAyuda"+id).style.display = "block";
        if(i==id){
          document.getElementById("textoAyuda"+i).style.display = "block";
        }else{
          document.getElementById("textoAyuda"+i).style.display = "none";
        }
        i++
      }
    }
    //para mostrar u ocultar caja de ayuda
    if(document.getElementById("hintDeportes") != null){
      if(primerId > 200){//si se trata de actividades masivas hacer
        
      }else if(primerId >100){//si se trata de actividades de ocio hacer
        document.getElementById("hintDeportes").style.display = "none";
        document.getElementById("hintOcio").style.display = "block";
      }else{//si se trata de un evento deportivo hacer
        document.getElementById("hintDeportes").style.display = "block";
        document.getElementById("hintOcio").style.display = "none";
      }
    }
  }
