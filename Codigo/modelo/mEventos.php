<?php
    ...
    class mEventos{
         ...
         * @param   numero  identificador de asistencia (introducir uno(1) para agregar al creador del evento)
         */
        static function actualizarListaParticipantes($ID_EVENTO,$ID_USUARIO,$ASISTIO = 0){
            $connect=conexionBaseDatos();
            $sql = "INSERT INTO PARTICIPANTES_EVENTO (ID_EVENTO,ID_PARTICIPANTE,ASISTIO)
                    VALUES ('$ID_EVENTO','$ID_USUARIO','$ASISTIO')";
            $result = $connect->query($sql);
        ...
        /**
         * Borrar un evento
         * @param   numero  id de usuario
         * @param   texto   id del evento
         * @return  numero  1 = se logro hacer el borrado | 0 = error al intentar realizar el borrado
         */
        static function borrarEvento($ID_USUARIO,$ID_EVENTO){
            $connect=conexionBaseDatos();
            ...
            echo "<script>console.warn('mEventos::borrarEvento)</script>";
            $connect->close();
            return $resultado;
        }
    }